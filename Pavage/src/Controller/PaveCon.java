...
public class PaveCon {
	...
	public void carre() {
		...
		//初始橫对称轴	Axe de symétrie transversal initial 	//可省略	Peut être omis
		pave.getSym_hor().add(new PointsM(-2*pave.getLongueur_pave() + Sym_ver_x, Sym_hor_y));
		pave.getSym_hor().add(new PointsM(2*pave.getLongueur_pave() + Sym_ver_x, Sym_hor_y));
		
		//初始纵对称轴	Axe de symétrie longitudinal initial	//可省略	Peut être omis
	...
	/**
	 * 查找对称信息 + 后期String需要拆分
	 * Trouver des informations symétriques + apres la chaine doit être divisée
	 * @param indice
	 * @return	String des informations symétriques
	 */
	...
	/**
	 * 添加一个点及对称点
	 * Ajouter un point et un point de symétrie
	 * @param indice
	 * @param Xpos
	 * @param Ypos
	 */
	public void addPoint(int indice, int Xpos, int Ypos) {
		...
		int indice1 = indice;	//indice: 加第1个点时的位置; indice1: indice的临时变量
								//indice: la position lorsque le premier point est ajouté; la variable temporaire de indice1: indice
		int indice11 = -1;	//加第2个点(对称点)时的位置
							//Position lorsque le deuxième point (point symétrique) est ajouté
		...
		if(chercherSommetIndice(Xpos, Ypos) == -1 && symPoint(indice1) != null) {
			...
			if(indice1 == 0) {
				...
				//更新pave的基本顶点位置列表	Mettre à jour la liste des positions de sommet de base des pavés
			...
			if(tempjurer[0].equals("h") && Integer.parseInt(tempjurer[1]) != -1 && Integer.parseInt(tempjurer[1]) != indice1 - 1) {
				...
				if(Integer.parseInt(tempjurer[1]) > indice1){	
					...
					//le changement symétrique index = tempindex1 + 1

					indice11 = Integer.parseInt(tempjurer[1]) + 2;
					if(indice11 >= pave.getPointList().size()) {
						if(tempjurer[2].equals("+"))
							pave.getPointList().add(new PointsM(Xpos, Ypos + pave.getLongueur_pave()));	//加入第2个点	Rejoignez le deuxième point
						else
							pave.getPointList().add(new PointsM(Xpos, Ypos - pave.getLongueur_pave()));	//加入第2个点	Rejoignez le deuxième point
					}else {
						...
						if(tempjurer[2].equals("+"))
							pave.getPointList().add(indice11, new PointsM(Xpos, Ypos + pave.getLongueur_pave()));	//加入第2个点	Rejoignez le deuxième point
						else
							pave.getPointList().add(indice11, new PointsM(Xpos, Ypos - pave.getLongueur_pave()));	//加入第2个点	Rejoignez le deuxième point
					}
				...
				System.out.println("L'adjonction du sommet a réussi!" + "\n");
			
			//纵轴对称	Symétrie verticale
			} else if(tempjurer[0].equals("v") && Integer.parseInt(tempjurer[1]) != -1 && Integer.parseInt(tempjurer[1]) != indice1 - 1) {

				//对称位置为tempindex2
				//La position symétrique est tempindex2
				if(Integer.parseInt(tempjurer[1]) > indice1) {
					//对称变化区间为<tempindex2,tempindex2 + 1>
					//L’intervalle de variation de symétrie est <tempindex2,tempindex2 + 1>
					//对称变化index = tempindex2 + 1
					//le changement symétrique index = tempindex2 + 1
					indice11 = Integer.parseInt(tempjurer[1]) + 2;
					if(indice11 >= pave.getPointList().size()) {
						if(tempjurer[2].equals("+"))
							pave.getPointList().add(new PointsM(Xpos + pave.getLongueur_pave(), Ypos));	//加入第2个点	Rejoignez le deuxième point
						else
							pave.getPointList().add(new PointsM(Xpos - pave.getLongueur_pave(), Ypos));	//加入第2个点	Rejoignez le deuxième point
					}else {
						...
						if(tempjurer[2].equals("+"))
							pave.getPointList().add(indice11, new PointsM(Xpos + pave.getLongueur_pave(), Ypos));	//加入第2个点	Rejoignez le deuxième point
						else
							pave.getPointList().add(indice11, new PointsM(Xpos - pave.getLongueur_pave(), Ypos));	//加入第2个点	Rejoignez le deuxième point
					}
				}else {
				...
				System.out.println("L'adjonction du sommet a échoué!" + "\n");
			}
		}
	}
	...
	/**
	 * 移动一个点及对称点
	 * Déplacer un point et un point de symétrie
	 * @param indice
	 * @param Xpos
	 * @param Ypos
	 */
	public void modifierPoint(int indice, int Xpos, int Ypos) {
		...
		if(jurer != -1) {
			System.out.println("C'est le sommet initial de base de Pave et ne peut pas être déplacé!" + "\n");	//注意：此为pave初始基本顶点，不能移动！
		}else {
			int indice1 = indice;	//indice1: indice临时存储变量
									//indice1: variable de stockage temporaire de l'indice
			int indice11 = -1;	//对称点的索引(indice)	Indice de points de symétrie (indice)
			...
			System.out.println("La déformation du sommet a réussi!" + "\n"); //顶点变形成功
		}
	}
	...
	/**
	 * 删除某个点及对称点
	 * Supprimer un point et un point de symétrie
	 * @param indice
	 */
	public void supprimerPoint(int indice) {
		...
		int jurer = pave.getBaseindice().indexOf(indice);
		if(jurer != -1) {
			System.out.println("C'est le sommet initial de base de Pave et ne peut pas être supprimé!" + "\n");	//注意：此为pave初始基本顶点，不能删除！
		}else {
			int indice1 = indice;	//indice1: indice临时存储变量
									//indice1: variable de stockage temporaire de l'indice
			int indice11 = -1;	//对称点的索引(indice)	Indice de points de symétrie (indice)
			...
			if(indice1 < indice11) {
				...
				pave.getPointList().remove(indice11);	//删除第2个点	Supprimer le deuxième point
				...
				pave.getPointList().remove(indice1);	//删除第2个点	Supprimer le deuxième point
			...
			System.out.println("La suppression du sommet a réussi!" + "\n"); //删除成功
		}
	}
	...
	/**
	 * 移动图形
	 * Déplacement de la pave
     * 判断点是否在多边形区域内
     * Déterminer si le point est dans la zone du polygone
     * @param pointLon 要判断的点的纵坐标	L'ordonnée du point à juger
     * @param pointLat 要判断的点的横坐标	L'abscisse du point à juger
     * @param lon 区域各顶点的纵坐标数组	Tableau d'ordonnées pour chaque sommet de la région
     * @param lat 区域各顶点的横坐标数组	Un tableau d'abscisses pour chaque sommet de la région
     * @return 是/否 true/false
     */
    public boolean isInPolygon(int pointLon, int pointLat, int[] lon, int[] lat) {
        // 将要判断的横纵坐标组成一个点
    	//Composer point selon des coordonnées horizontales et verticales à juger
        Point2D.Double point = new Point2D.Double(pointLon, pointLat);
        // 将区域各顶点的横纵坐标放到一个点集合里面
        //Mettez les coordonnées horizontales et verticales de chaque sommet de la surface en un ensemble de points
     ...
     * @param point 要判断的点的横纵坐标	Les coordonnées horizontales et verticales du point à juger
     * @param polygon 组成的顶点坐标集合	Ensemble de coordonnées de sommet
     * @return 是/否	true/false
     */
    private boolean check(Point2D.Double point, List<Point2D.Double> polygon) {
        ...
        //Ajouter un point à la trajectoire en déplacant les coordonnées spécifiées (spécifiées en double précision)
        peneralPath.moveTo(first.x, first.y);
        polygon.remove(0);
        for (Point2D.Double d : polygon) {
            // 通过绘制一条从当前坐标到新指定坐标（以双精度指定）的直线，将一个点添加到路径中。
        	//Ajoutez un point au tracé en tracant une ligne entre les coordonnées actuelles et les nouvelles coordonnées (spécifiées en double précision).
            peneralPath.lineTo(d.x, d.y);
        }
        // 将几何多边形封闭
        //Fermer le polygone géométrique
        peneralPath.lineTo(first.x, first.y);
        peneralPath.closePath();
        // 测试指定的 Point2D 是否在 Shape 的边界内。
        //Teste si le Point2D spécifié est dans les limites de la forme.
